...
class ConvDisRNN(nn.Module):
    def __init__(self, batch_size, in_channels, out_channels, kernel_size, num_classes, time_len):
        ...
        self.fc_lstm1 = nn.Linear((self.time_len - 1) * 16, num_classes)
        self.fc_lstm2 = nn.Linear((self.time_len - 1) * 16, num_classes)
    ...
    def forward(self, x):
        ...
        h_lstm = self.fc_lstm2(h_lstm)

        out1 = flow1 + h_lstm
        flow1 = self.softmax(flow1)
        h_lstm1 = self.softmax(h_lstm)
        out1 = self.softmax(out1)
        return flow1, h_lstm1, out1
